...
% footnote formatting
%
% Default \symbolfootnotes / \arabicfootnotes are no-ops so the LaTeXML
% conversion (which doesn't typeset footnote markers — they're emitted as
% semantic XML and styled by CSS) still resolves the document-body calls.
% In print mode they are redefined below to switch numbering style and
% the print-only footmisc/@makefnmark machinery is loaded.
%
% The \newif booleans must be declared BEFORE the \ifnotlatexml guard:
% LaTeXML's skip-mode scanner counts \ifXXX tokens it encounters even
% inside the body of \renewcommand, so any \newif inside the skipped
% block would throw off \fi matching.
...
\renewcommand{\symbolfootnotes}{%
  \arabicfootnotesfalse
  \renewcommand{\thefootnote}{\fnsymbol{footnote}}%
}

\renewcommand{\arabicfootnotes}{%
  \arabicfootnotestrue
